...
public class CaffeineUtil {
    ...
    private static <K, V> Caffeine<K, V> commonCaffeine(Integer minSize,
        ...
        if (Objects.equals(minExpireTime, maxExpireTime)) {
            ...
            caffeine.expireAfter(new MyDefaultExpiry<>(minExpireTime, maxExpireTime, expireTimeUnit));
        }
        return caffeine;
    }
     ...
     * 自定义过期策略
     */
    static class MyDefaultExpiry<K, V> implements Expiry<K, V> {
        private final Integer minExpirationTime;
        private final Integer maxExpirationTime;
        private final TimeUnit expireTimeUnit;

        private MyDefaultExpiry(Integer minExpireTime, Integer maxExpireTime, TimeUnit expireTimeUnit) {
