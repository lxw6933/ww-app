...
import static com.ww.app.common.utils.CollectionUtils.findFirst;

/**
 * @author ww
 * @create 2025-09-25 15:43
 * @description: 地址区域工具类
 */
@Slf4j
public class AreaUtils {

    private final static List<Integer> ID_PARENT = Arrays.asList(Area.ID_GLOBAL, Area.ID_CHINA);
    ...
    public static List<String> getAreaNodePathList(List<Area> areas) {
        ...
        areas.forEach(area -> getAreaNodePathList(area, StrUtil.EMPTY, paths));
        return paths;
    }
    ...
    private static void getAreaNodePathList(Area node, String path, List<String> paths) {
        ...
        // 构建当前节点的路径
        String currentPath = path.isEmpty() ? node.getName() : path + StrUtil.SLASH + node.getName();
    ...
    public static String format(Integer id, String separator) {
        ...
        for (int i = 0; i < AreaTypeEnum.values().length; i++) {
            ...
            area = area.getParent();
            // 跳过父节点为中国的情况
            if (area == null || ID_PARENT.contains(area.getId())) {
                break;
            }
