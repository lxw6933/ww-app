...
public class RedisPvUvAutoConfiguration {
     ...
     */
    private final Set<RedisPvUvManager> managers = Collections.newSetFromMap(new ConcurrentHashMap<>());
    ...
    @Bean
    @ConditionalOnMissingBean
    public RedisPvUvManager redisPvUvManager(RedisPvUvStorage redisPvUvStorage, PvUvRedisKeyBuilder pvUvRedisKeyBuilder) {
        RedisPvUvManager manager = new RedisPvUvManager(redisPvUvStorage, pvUvRedisKeyBuilder);
    ...
    public void destroy() {
        if (!managers.isEmpty()) {
            ...
            for (RedisPvUvManager manager : managers) {
                try {
                    manager.shutdown();
                } catch (Exception e) {
