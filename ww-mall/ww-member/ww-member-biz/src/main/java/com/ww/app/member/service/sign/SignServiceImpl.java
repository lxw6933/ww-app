...
public class SignServiceImpl implements SignService {
     ...
     * 执行指定日期签到（包括补签）
     */
    private int doSpecificDateSign(LocalDate date, ClientUser clientUser) {
        // 获取签到策略
        SignStrategy strategy = signStrategyFactory.getDefaultStrategy();
        ...
        // 检查剩余补签次数
        int remainingCount = getRemainingResignCount(clientUser, date);
        if (remainingCount <= 0) {
            throw new ApiException("本月补签次数已用完");
        }
        // 构建签到Key
        String signKey = signRedisKeyBuilder.buildMonthlySignPrefixKey(clientUser.getId(), date);
        // 获取偏移量
    ...
    public int getSignCount(LocalDate date, ClientUser clientUser) {
        SignStrategy strategy = signStrategyFactory.getDefaultStrategy();
        return strategy.getSignCount(date, clientUser);
    }
    ...
    public Map<String, Boolean> getSignInfo(LocalDate date, ClientUser clientUser) {
        SignStrategy strategy = signStrategyFactory.getDefaultStrategy();
        return strategy.getSignInfo(date, clientUser);
    }
     ...
     * 检查用户在指定日期是否已签到
     */
    private boolean isSignedOn(LocalDate signDate, ClientUser clientUser) {
        // 构建签到Key
